...
**Boundary rule**: a module never imports another bounded context's JPA entity or repository directly. Cross-context access goes through an `xxx.api` package (DTOs + port interfaces) or, preferably, domain events. Meant to be enforced with **ArchUnit** tests from module 1 — this is the concrete mechanism that keeps "extract a service later" (blueprint §8) realistic instead of aspirational; retrofitting boundaries after several milestones is expensive. **Not yet true in practice**: as of Epic 1.6 the reactor is 5 modules deep on convention alone, with no ArchUnit suite written — see ADR 0001's status update and risk 5 below.
...
| 3.2 Policy/scorecard/pricing admin | Immutable, publish-only `PolicyVersion`/`ScorecardVersion`/`PricingVersion`; `POST /policies` + `POST /policies/{id}/publish` (`POLICY_ADMIN`) | Publishing a new version doesn't change which version an already-recorded decision references |
...
| 3.4 Decision engine integration | Synchronous WebClient call (Resilience4j timeout/circuit-breaker), weighted-band evaluation, `Decision` (all 4 outcomes) with full version/evidence traceability + `credit_score_model_version` captured | All 4 outcomes reachable by tuning synthetic input; a forced credit-score outage yields `PENDING_REVIEW`/ops task, never a silent wrong decision |
...
5. **Module-boundary erosion** — direct cross-module JPA entity/repository access defeats §8's "extract later" goal. Fix early with `xxx.api` packages + ArchUnit; retrofitting boundaries later is expensive. **Live, not hypothetical**: confirmed during Epic 1.6's ADR audit that this is already the actual state — 5 modules, `xxx.api` packages followed by convention, zero ArchUnit enforcement (see ADR 0001). Must close at the start of Milestone 2, before `workflow` adds a 6th module.
