...
**Boundary rule**: a module never imports another bounded context's JPA entity or repository directly. Cross-context access goes through an `xxx.api` package (DTOs + port interfaces) or, preferably, domain events. Enforced with **ArchUnit** tests from module 1 — this is the concrete mechanism that keeps "extract a service later" (blueprint §8) realistic instead of aspirational; retrofitting boundaries after several milestones is expensive.
...
5. **Module-boundary erosion** — direct cross-module JPA entity/repository access defeats §8's "extract later" goal. Fix early with `xxx.api` packages + ArchUnit; retrofitting boundaries later is expensive.
